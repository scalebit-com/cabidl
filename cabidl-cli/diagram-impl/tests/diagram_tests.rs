...
#[test]
fn generate_with_known_type_writes_file() {
    let fs = InMemoryFilesystem::new();
    let diagram = DiagramImpl::new(
        vec![Box::new(GraphvizProvider)],
        Box::new(fs),
    );

    let output = Path::new("/output.dot");
    let result = diagram.generate(&test_system(), "graphviz", output);
    assert!(result.is_ok());
}
...
fn generate_with_unknown_type_returns_error() {
    ...
    let output = Path::new("/output.dot");
    let result = diagram.generate(&test_system(), "mermaid", output);
