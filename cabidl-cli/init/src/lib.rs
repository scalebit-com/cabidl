...
pub trait Init {
    ...
    ///
    /// Decompresses the embedded templates archive in memory and streams
    /// the selected template's contents (excluding `template.yaml`) to
    /// `target_dir` via the Filesystem boundary.
    fn scaffold(
        &self,
        template_name: &str,
        target_dir: &Path,
